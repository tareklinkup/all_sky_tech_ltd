...
class Sales extends CI_Controller
{
    ...
    public function getSalesSerials()
    {
    ...
    //     $getCusInfo = $this->db->query("select SalseCustomer_IDNo,SaleMaster_InvoiceNo from tbl_salesmaster where SaleMaster_SlNo=? ", $SaleMaster_IDNo)->row();
    ...
    //                  <td style="text-align: center;">' . $value->SaleDetails_Rate . '</td>
    //                  <td style="display: flex;align-content: center;justify-content: center;"><button style="width: 118px;padding: 7px;margin: 2px;border: navajowhite;" class="btn btn-success btn-sm" id="parchase_return_btn" data-id="' . $value->ps_imei_number . '" data-prod_id="' . $value->Product_SlNo . '" data-amount="' . $value->SaleDetails_Rate . '" data-cusId="' . $getCusInfo->SalseCustomer_IDNo . '"
    //                  data-invoice="' . $getCusInfo->SaleMaster_InvoiceNo . '"> Return</button></td>
    //              </tr>';
    //     }
    //     $html .= '</table>';
    //     echo $html;
    ...
    public function sale_imei_return()
    {
        ...
        try {
            ...
            $salesReturn = array(
                ...
                'SaleReturn_ReturnDate'   => date('Y-m-d'),
                'SaleReturn_ReturnAmount' => $data->sale_return_amount,
                'SaleReturn_Description'  => $data->note,
                'Status'                  => 'a',
            ...
            $returnDetails = array(
                ...
                'SaleReturnDetailsProduct_SlNo'    => $data->ps_prod_id,
                'SaleReturnDetails_ReturnQuantity' => 1,
                'SaleReturnDetails_ReturnAmount'   => $data->sale_return_amount,
                'Status'                           => 'a',
                'AddBy'                            => $this->session->userdata("FullName"),
                'AddTime'                          => date('Y-m-d H:i:s'),
            ...
            $salesReturnDetailsId = $this->db->insert_id();

            $datas = array(
                'ps_s_r_status'          => 'yes',
                'ps_s_status'            => 'no',
                'ps_s_r_amount'          => $data->sale_return_amount,
                'cus_id'                 => $data->SalseCustomer_IDNo,
                'sale_return_details_id' => $salesReturnDetailsId
            );
            $this->db->where('ps_imei_number', $data->ps_imei_number);
            $result = $this->db->update('tbl_product_serial_numbers', $datas);
    ...
    public function getImeiHistory()
    {

        $data = json_decode($this->input->raw_input_stream);

        $clauses = '';
        if (isset($data->ps_id) && $data->ps_id != '') {
            $clauses .= " and psn.ps_id = '$data->ps_id'";
        }
    ...
    {
        $branch_id = $this->session->userdata("BRANCHid");
        $imei = $this->db->query("SELECT ps.ps_imei_number,ps.purchase_date,ps.ps_prod_id,ps.sales_details_id,tsm.SaleMaster_InvoiceNo,tsm.SaleMaster_SaleDate,tsm.reference,p.*,u.Unit_Name,pc.ProductCategory_Name
        FROM  tbl_product_serial_numbers as ps
        INNER JOIN tbl_product as p ON ps.ps_prod_id = p.Product_SlNo
        INNER JOIN tbl_unit as u ON p.Unit_ID= u.Unit_SlNo
        INNER JOIN tbl_productcategory as pc ON p.ProductCategory_ID= pc.ProductCategory_SlNo
        INNER JOIN tbl_saledetails tsd ON tsd.SaleDetails_SlNo = ps.sales_details_id
        INNER JOIN tbl_salesmaster tsm ON tsm.SaleMaster_SlNo = tsd.SaleMaster_IDNo
        WHERE ps.ps_s_status ='yes' AND ps.ps_brunch_id='$branch_id'")->result();
        echo json_encode($imei);
    }
            ...
            foreach ($data->cart as $cartProduct) {
                ...
                if (count($cartProduct->SerialStore) > 0) {
                    foreach ($cartProduct->SerialStore as $key => $value) {
                        ...
                        $this->db->where('ps_imei_number', $value->imeiNumber);
                        $this->db->update('tbl_product_serial_numbers', $data);
                    }
                }
            }

            // $currentDue = $previousDue_tk + ($total_tk - $paid_tk);
            // // Send sms
    ...
    public function updateSales()
    {
        ...
        try {
            ...
            foreach ($data->cart as $cartProduct) {
                ...
                if (count($cartProduct->SerialStore) > 0) {
                    foreach ($cartProduct->SerialStore as $keyimei => $valueimei) {
                        $dataArr = array(
                            'ps_s_r_status' => 'no',
                            'ps_s_r_amount' => 0,
                            'ps_s_status' => 'yes',
                        ...
                        $this->db->where('ps_imei_number', $valueimei->imeiNumber);
                        $this->db->update('tbl_product_serial_numbers', $dataArr);
                    }
                }
            }
    ...
    public function getSaleDetailsForReturn()
    {
        ...
        $saleDetails = array_map(function ($saleDetail) {
            $saleDetail->serial = $this->db->query("
                ...
                left join tbl_saledetails sd on sd.SaleDetails_SlNo = ps.sales_details_id
    ...
    public function addSalesReturn()
    {
        ...
        try {
            ...
            foreach ($data->serialStore as $product) {
                ...
                $this->db->insert('tbl_salereturndetails', $returnDetails);
                $saleReturnDetailsId = $this->db->insert_id();
                ...
                $this->db->query("
                    ...
                    ps_s_r_amount = ?,
                    sale_return_details_id = ?
                    where ps_id = ?
                    and ps_prod_id = ?
                ", [$product->returnAmount, $saleReturnDetailsId, $product->id, $product->productId]);
            }
    ...
    public function getSaleReturnDetails()
    {
        ...
        $returnDetails = $this->db->query("
            select
                ...
                psn.ps_imei_number
            from tbl_salereturndetails srd            
            left join tbl_product_serial_numbers psn on psn.sale_return_details_id = srd.SaleReturnDetails_SlNo
        ...
        //     WHERE ps_s_r_status = 'yes'
        //     and ps_prod_id = ?", $value->SaleReturnDetailsProduct_SlNo)->result();
...
<?php
    ...
    public function  deleteSales()
    {
        ...
        try {
            ...
            $saleReturn = $this->db->select('*')->where('SaleMaster_InvoiceNo', $sale->SaleMaster_InvoiceNo)->get('tbl_salereturn')->result();
            if (count($saleReturn) > 0) {
                $res = ['success' => false, 'message' => 'Sale Return Founded! Delete Not Possible.'];
                echo json_encode($res);
                exit;
            }

            /*Get Sale Details Data*/
            $saleDetails = $this->db->select('*')->where('SaleMaster_IDNo', $saleId)->get('tbl_saledetails')->result();
        ...
        foreach ($sales as $sale) {
            $sale->saleDetails = $this->db->query("
                select
                    ...
                    case when sd.Discount_amount is null then 0
                    else (sd.SaleDetails_Rate * (sd.Discount_amount/100))
                    end as discount,
