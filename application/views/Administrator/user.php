...
$getBranch = $this->db->query("SELECT * FROM tbl_brunch where status = 'a' order by Brunch_name asc");
$branches = $getBranch->result();

$getEmployee = $this->db->query("SELECT * FROM tbl_employee where status = 'a' order by Employee_ID asc");
$employees = $getEmployee->result();

$getAllUser = $this->db->query("SELECT u.User_SlNo, u.User_ID, u.FullName, u.User_Name, u.UserEmail, u.userBrunch_id, u.UserType, u.status AS userstatus, br.brunch_id, br.Brunch_name FROM tbl_user AS u LEFT JOIN tbl_brunch AS br ON br.brunch_id = u.userBrunch_id");
		...
		<div class="col-xs-12">
			...
			<div class="form-horizontal">
				<div class="col-sm-6">
					...
					<div class="form-group">
						<label class="col-sm-4 control-label" for="Brunch"> Select Employee </label>
						<label class="col-sm-1 control-label">:</label>
						<div class="col-sm-6">
							<select class="chosen-select form-control" name="employee" id="employee" data-placeholder="Choose a Employee..." required>
								<option value=""></option>
								<?php
								foreach ($employees as $employee) { ?>
									<option value="<?= $employee->Employee_SlNo; ?>"><?= $employee->Employee_ID . ' - ' . $employee->Employee_Name  ?></option>
								<?php } ?>
							</select>
							<span id="employee_error"></span>
						</div>
					</div>
							...
							<tr>
								...
								<td>
									<div class="hidden-sm hidden-xs action-buttons">
										<a class="blue" href="<?php echo base_url() . 'userEdit/' . $row->User_SlNo; ?>" onclick="return confirm('Are you sure to edit this user?');">
...
<script type="text/javascript">
	function submit() {
		...
		if (Brunch == "") {
			...
			$("#branch_error").html("Employee Required!");
			return false;
		} else {
			$("#branch_error").html("");
		...
		if (Emp == "") {
			$("#employee_error").css("color", "red");
			$("#employee_error").html("Employee Required!");
			return false;
		} else {
			// $("#employee").css("border-color", "green");
			$("#employee_error").html("");
		}

		var type = $("#type").val();
		if (type == "") {
			$("#user_type_error").css("color", "red");
			$("#user_type_error").html("Employee Required!");
			return false;
		} else {
		...
		var inputdata = 'username=' + username + '&rePassword=' + rePassword + '&txtFirstName=' + txtFirstName + '&user_email=' + user_email + '&Brunch=' + Brunch + '&type=' + type + '&employee=' + Emp;
