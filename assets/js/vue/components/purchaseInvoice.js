const purchaseInvoice = Vue.component("purchase-invoice", {
            ...
            <div id="invoiceContent">
                        ...
                        <table _a584de>
                            ...
                            <tbody>
                                <tr v-for="(product, sl) in cart">
                                    <td>{{ sl + 1 }}</td>
                                    <td v-if="product.serial.length > 0">{{ product.Product_Name }} <br>
                                    ({{ product.serial.map(obj => obj.ps_imei_number).join(', ') }})
                                    </td>
                ...
                <div class="row">
                    <div class="col-xs-12">
                        ...
                        <strong>Guaranty and Warranty: </strong>
                        <p style="white-space: pre-line">{{ purchase.guaranty_warranty }}</p>
                    </div>
                </div>
            </div>
        </div>
  ...
  methods: {
    ...
    async getPurchaseImei() {
      await axios
        .post("/get_purchases_imei", { purchaseId: this.purchase_id })
        .then((res) => {
