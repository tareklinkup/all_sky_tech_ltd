...
class Model_Table extends CI_Model
{
    ...
    public function generateSalesInvoice()
    {
        ...
        $invoice = date('ymd') . $branchNo . "001";
        $yearMDayBranch = date('ymd') . $branchNo;
        $sales = $this->db->query("select SaleMaster_InvoiceNo from tbl_salesmaster sm where sm.SaleMaster_InvoiceNo like '$yearMDayBranch%' order by SaleMaster_InvoiceNo desc limit 1")->row();
        if ($sales) {
            $invoice = $sales->SaleMaster_InvoiceNo + 1;
        }
    ...
    {
        $newEmpId = "E1001";
        $lastEmp = $this->db->query("select * from tbl_employee order by Employee_SlNo desc limit 1")->row();
        if ($lastEmp) {
            $newEmpId = 'E' . (substr($lastEmp->Employee_ID, 1) + 1);
        }
    ...
    public function getTransactionSummary($date = null)
    {
        $transactionSummary = $this->db->query("
            ...
            /* total */
            (
                select received_sales + received_customer + received_supplier + received_cash + bank_withdraw + invest_received + loan_received + loan_initial_balance
            ) as total_in,
            (
                select paid_purchase + paid_customer + paid_supplier + paid_cash + bank_deposit + employee_payment + invest_payment + loan_payment
            ) as total_out,
            (
