			...
			<table class="table table-bordered">
				...
				<tbody>
					<template v-for="(product, sl) in cart">
						...
						<tr v-for="(pur, sl) in product.serial.filter(item => item.ps_p_r_status == 'no')">
							<td>{{ sl + 1 }}</td>
							<td>{{ product.Product_Name }} ({{ pur.ps_imei_number }})</td>
							<td>1</td>
							<td>{{ pur.purchase_rate }}</td>
							<td>{{ 0 }}</td>
							<td>{{ 0 }}</td>
							<td><a href="" class="btn rt-btn btn-sm" @click.prevent="serialProductReturn(pur,sl)">Return</a></td>
							<td>{{ pur.purchase_total }}</td>
							<td v-if="pur.select" style="background: rgb(255 240 64);line-height: 25px;">{{ 'Selected' }}</td>
							<td v-else></td>
						</tr>
					</template>
				</tbody>
...
<script>
	...
	new Vue({
		...
		methods: {
			...
			calculateTotal() {
				let returnTotal = this.cart.reduce((prev, cur) => {
					return prev + (cur.return_amount ? parseFloat(cur.return_amount) : 0.00)
				}, 0);
				let serialReturnTotal = this.serialCart.reduce((prev, cur) => {
					return prev + (cur.returnAmount ? parseFloat(cur.returnAmount) : 0.00)
				}, 0);
				this.purchaseReturn.total = (parseFloat(returnTotal) + parseFloat(serialReturnTotal)).toFixed(2)
			},
			savePurchaseReturn() {
			...
			serialProductReturn(product, sl) {
				this.cart[0].serial[sl].select = true;
				let serial = {
					id: product.ps_id,
					productId: product.ps_prod_id,
					serialNumber: product.ps_imei_number,
					purchaseId: product.purchase_details_id,
					returnAmount: product.purchase_total,
					quantity: 1
				}
				console.log(this.cart[0].serial[sl]);
