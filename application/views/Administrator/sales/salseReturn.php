					...
					<table class="table table-bordered">
						...
						<tbody>
							<template v-for="(product, sl) in cart">
								<tr v-if="product.serial.length == 0">
									<td>{{ sl + 1 }}</td>
								...
								<tr v-else v-for="(sel, sl) in product.serial.filter(item => item.ps_s_r_status == 'no')">
									<td>{{ sl + 1 }}</td>
									<td>{{ product.Product_Name }} ({{ sel.ps_serial_number }})</td>
									<td>1</td>
									<td>{{ sel.SaleDetails_Rate }}</td>
									<td>{{ 0 }}</td>
									<td>{{ 0 }}</td>
									<td><button class="btn btn-sm rt-btn" @click="serialProductReturn(sel)">return</button></td>
									<td><input type="text" v-model="sel.SaleDetails_Rate" v-on:input="serialProductReturnAmount(sl, sel)"></td>
									<td>{{ ''}}</td>
								</tr>
							</template>
						</tbody>
	...
	new Vue({
		...
		methods: {
			...
			productReturnTotal(ind) {
				...
				this.cart[ind].return_amount = parseFloat(this.cart[ind].return_quantity) * (parseFloat(this.cart[ind].return_rate) - parseFloat(this.cart[ind].SaleDetails_Discount));
				this.calculateTotal();
			},
			calculateTotal() {
				let cartReturn = this.cart.reduce((prev, cur) => {
					return prev + (cur.return_amount ? parseFloat(cur.return_amount) : 0.00)
				}, 0);
				let serialReturn = this.serialCart.reduce((prev, cur) => {
					return prev + (cur.returnAmount ? parseFloat(cur.returnAmount) : 0.00)
				}, 0);
				this.salesReturn.total = (parseFloat(cartReturn) + parseFloat(serialReturn)).toFixed(2);
			},
			saveSalesReturn() {
				let filteredCart = this.cart.filter(product => product.return_quantity > 0 && product.return_rate > 0);

				if (filteredCart.length == 0 && this.serialCart.length == 0) {
			...
			serialProductReturn(product) {
				let serial = {
					id: product.ps_id,
					productId: product.ps_prod_id,
					serialNumber: product.ps_imei_number,
					salesId: product.sales_details_id,
					returnAmount: product.SaleDetails_Rate,
					quantity: 1
				}
				console.log(product, serial);

				let serialProduct = this.serialCart.findIndex(item => item.serialNumber == product.ps_imei_number)
				...
				this.serialCart.push(serial);
				this.calculateTotal()
			},
			serialProductReturnAmount(ind, product) {
				let serial = {
					id: product.ps_id,
					productId: product.ps_prod_id,
					serialNumber: product.ps_serial_number,
					salesId: product.sales_details_id,
					returnAmount: product.Product_SellingPrice,
					quantity: 1
				}
				let serialProduct = this.serialCart.findIndex(item => item.serialNumber == product.ps_serial_number)

				if (serialProduct > -1) {
					this.serialCart.splice(serialProduct, 1);
				}
				this.serialCart.push(serial)
				this.calculateTotal()

				if (this.serialCart.length > 0) {
					this.serialCart[ind].returnAmount = product.Product_SellingPrice;
				}
			},
			async getReturn() {
