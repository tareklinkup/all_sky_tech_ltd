...
class Purchase extends CI_Controller
{
    ...
    public function getPurchases()
    {
        ...
        if (isset($data->purchaseId) && $data->purchaseId != null) {
            ...
            $purchaseDetails = array_map(function ($purchaseDetails) {
                $purchaseDetails->serial = $this->db->query("SELECT * FROM tbl_product_serial_numbers WHERE purchase_details_id = ?", $purchaseDetails->PurchaseDetails_SlNo)->result();
    ...
    public function getPurchaseDetailsForReturn()
    {
        ...
        $purchaseDetails = array_map(function ($purchaseDetails) {
            $purchaseDetails->serial = $this->db->query("
                select 
                    *
                from tbl_product_serial_numbers 
                where purchase_details_id=?", $purchaseDetails->PurchaseDetails_SlNo)->result();
            return $purchaseDetails;
        }, $purchaseDetails);
    ...
    public function addPurchaseReturn()
    {
        ...
        try {
            ...
            foreach ($data->serialStore as $product) {
                $returnDetails = array(
                    'PurchaseReturn_SlNo' => $purchaseReturnId,
                    'PurchaseReturnDetailsProduct_SlNo' => $product->productId,
                    'PurchaseReturnDetails_ReturnQuantity' => $product->quantity,
                    'PurchaseReturnDetails_ReturnAmount' => $product->returnAmount,
                    'Status' => 'a',
                    'AddBy' => $this->session->userdata("FullName"),
                    'AddTime' => date('Y-m-d H:i:s'),
                    'PurchaseReturnDetails_brachid' => $this->session->userdata('BRANCHid')
                ...
                $this->db->query("
                    ...
                    set ps_p_r_status = 'yes', ps_p_r_amount = ?
                    where ps_prod_id = ? and ps_id = ? and purchase_details_id = ?
                ", [$product->returnAmount, $product->productId, $product->id, $product->purchaseId]);
            }
    ...
    public function getPurchaseReturns()
    {
        ...
        if (isset($data->purchaseInvoice) && $data->purchaseInvoice != '') {
            $clauses .= " and pr.PurchaseMaster_InvoiceNo = '$data->purchaseInvoice'";
        }
    ...
    public function getPurchaseReturnDetails()
    {
        ...
        $returnDetails = $this->db->query("
            select 
                ...
                psn.ps_imei_number
            from tbl_purchasereturndetails prd
            left join tbl_product_serial_numbers psn on psn.purchase_return_details_id = prd.PurchaseReturnDetails_SlNo
        ...
        // $returnDetails = array_map(function ($value) {
        //     $value->IMEI = $this->db->query("SELECT ps_imei_number
        //     FROM tbl_product_serial_numbers
        //     WHERE ps_p_r_status = 'yes'
        //     and purchase_return_details_id = ?", [$value->PurchaseReturnDetails_SlNo])->result();
    ...
    {
        $purchaseInvoice = $this->db->query("select PurchaseMaster_InvoiceNo from tbl_purchasemaster where PurchaseMaster_SlNo = ?", $purchaseId)->row()->PurchaseMaster_InvoiceNo;
        $CheckReturn = $this->db->query("SELECT * FROM `tbl_purchasereturn` WHERE `Status` = 'a' and `PurchaseReturn_brunchID` = ? and `PurchaseMaster_InvoiceNo` = ?", [$this->session->userdata('BRANCHid'), $purchaseInvoice])->result();

        if (count($CheckReturn) > 0) {
            echo 'Return Found!. Edit not possible';
            exit;
        }
    ...
    public function purchase_imei_return()
    {
        ...
        try {
            ...
            $purchaseReturn = array(
                ...
                'Supplier_IDdNo'              => $data->Supplier_SlNo,
                'PurchaseReturn_ReturnDate'   => date('Y-m-d'),
                'PurchaseReturn_ReturnAmount' => $data->purchse_return_amount,
                'PurchaseReturn_Description'  => $data->note,
                'Status'                      => 'a',
                'AddBy'                       => $this->session->userdata("FullName"),
                'AddTime'                     => date('Y-m-d H:i:s'),
                'PurchaseReturn_brunchID'     => $this->session->userdata('BRANCHid')
            ...
            $returnDetails = array(
                'PurchaseReturn_SlNo'                  => $purchaseReturnId,
                'PurchaseReturnDetailsProduct_SlNo'    => $data->ps_prod_id,
                'PurchaseReturnDetails_ReturnQuantity' => 1,
                'PurchaseReturnDetails_ReturnAmount'   => $data->purchse_return_amount,
                'Status'                               => 'a',
                'AddBy'                                => $this->session->userdata("FullName"),
                'AddTime'                              => date('Y-m-d H:i:s'),
                'PurchaseReturnDetails_brachid'        => $this->session->userdata('BRANCHid')
            ...
            $this->db->insert('tbl_purchasereturndetails', $returnDetails);
            $purchaseReturnDetailsId = $this->db->insert_id();

            $datas = array(
                'ps_p_r_status'      => 'yes',
                'ps_p_r_amount'      => $data->purchse_return_amount,
                'purchase_return_details_id' => $purchaseReturnDetailsId
            );
            $this->db->where('ps_imei_number', $data->ps_imei_number)->update('tbl_product_serial_numbers', $datas);
            ...
            $result = $this->db->query("
                ...
                where product_id = ?
                and branch_id = ?
            ", [1, $data->ps_prod_id, $this->session->userdata('BRANCHid')]);

            $res = ['success' => true, 'message' => 'Return Successfull'];
            $this->db->trans_commit();
        } catch (\Exception $e) {
            $this->db->trans_rollback();
            $res = ['success' => false, 'message' => 'Return failed!'];
        }
    ...
    public function getPurchaseSerialNumber()
    {
        $serialIMEIs = $this->db->query("SELECT
        psn.ps_imei_number,
        psn.ps_prod_id,
        psn.purchase_details_id,
        p.Product_Name,
        p.Product_Code,
        pd.PurchaseDetails_Rate,
        pd.PurchaseDetails_Rate as purchse_return_amount,
        ...
        WHERE psn.ps_s_status = 'no'
        and psn.ps_p_r_status = 'no'
    ...
    public function addPurchase()
    {
        ...
        try {
            ...
            foreach ($data->cartProducts as $product) {
                ...
                if (isset($product->IMEICartStore)) {
                    foreach ($product->IMEICartStore as $imeikey => $imeivalue) {

                        $purchase_discount = $imeivalue->discount / $imeivalue->totalQty;
                        $p_RateAfterDis = $imeivalue->purchaseRate - $purchase_discount;
    ...
    public function updatePurchase()
    {
        ...
        try {
            ...
            $this->db->query("delete from tbl_product_serial_numbers where ps_purchase_inv = ?", $invoice);
    ...
    public function  deletePurchase()
    {
        ...
        try {
            ...
            foreach ($purchaseDetails as $detail) {
                ...
                if ($detail->PurchaseDetails_TotalQuantity > $totalQty) {
    ...
    function addDamage()
    {
        ...
        try {
            ...
            if ($data->serialId != '') {
                $this->db->where('ps_id', $data->serialId)->set(['damage_status' => 'yes', 'damage_details_id' => $damageDetailsId])->update('tbl_product_serial_numbers');
            }
    ...
    public function updateDamage()
    {
        ...
        try {
            ...
            $this->db->where('damage_details_id', $oldProduct->DamageDetails_SlNo)->set(['damage_status' => 'no', 'damage_details_id' => 0])->update('tbl_product_serial_numbers');
            ...
            if ($data->serialId != '') {
                $this->db->where('ps_id', $data->serialId)->set(['damage_status' => 'yes', 'damage_details_id' => $oldProduct->DamageDetails_SlNo])->update('tbl_product_serial_numbers');
            }
    ...
    public function getDamages()
    {
        ...
        foreach ($damages as $key => $value) {
            if ($value->is_serial == true) {
                $value->damageSerial = $this->db->query("SELECT * FROM tbl_product_serial_numbers WHERE damage_status = 'yes' and damage_details_id =  ?", $value->DamageDetails_SlNo)->row();
            }
        }
