...
class Transfer extends CI_Controller
{
    ...
    public function addProductTransfer()
    {
        ...
        try {
            ...
            foreach ($data->cart as $cartProduct) {
                ...
                $transferDetailsId = $this->db->insert_id();
                ...
                if (count($cartProduct->SerialStore) > 0) {
                    foreach ($cartProduct->SerialStore as $key => $sProduct) {
                        $insertArray = array(
                            'transfer_id'  => $transferDetailsId,
                            'ps_brunch_id' => $data->transfer->transfer_to,
                        );
                        $this->db->where('ps_imei_number', $sProduct->ps_imei_number)->update('tbl_product_serial_numbers', $insertArray);
                    }
                }
            }
    ...
    public function updateProductTransfer()
    {
        ...
        try {
            ...
            foreach ($oldTransferDetails as $oldDetails) {
                ...
                $this->db->where('transfer_id', $oldDetails->transferdetails_id)->update('tbl_product_serial_numbers', $insertArray);
            }
    ...
    public function getTransferDetails()
    {
        ...
        foreach ($transferDetails as $key => $value) {
            $value->serials = $this->db->query("SELECT * FROM `tbl_product_serial_numbers` WHERE `transfer_id` = ?", $value->transferdetails_id)->result();
        }
    ...
    public function transferInvoice($transferId)
    {
        ...
        foreach ($data['transferDetails'] as $key => $value) {
            $value->serials = $this->db->query("SELECT * FROM `tbl_product_serial_numbers` WHERE `transfer_id` = ?", $value->transferdetails_id)->result();
        }
